...
- **`release.yml`** — triggers on `push` to master (i.e., after PR merge). Reads version from `package.json`, checks if a `v{version}` tag already exists. If not → generates release notes, builds artifacts on Windows + Linux, uploads them as workflow artifacts, and a final `publish` job creates a draft GitHub Release with all assets then flips it to published (immutable-release safe; publishing creates the tag). If the tag exists → skips silently.
...
**Release scripts** (`scripts/rc-tag.js`, `scripts/release-tag.js`): create a `release/v{version}` branch, bump `package.json` (no local git tags), push, and auto-create a PR via `gh pr create`. Run with `npm run rc:patch` / `npm run release:patch` etc. **`npm run release:final` promotes the current RC to its stable version** (`0.1.4-rc.2` → `0.1.4`) — always use it to finalize an RC cycle, so the stable tag shares its base with the RC tags and release notes report the RC range. `release:patch|minor|major` are only for direct stable releases with no RC phase (they bump past the current version).

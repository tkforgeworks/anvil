...
- **`ci.yml`** — triggers on pushes to non-master branches and `pull_request` to master. Runs `validate` job (`npm audit --omit=dev` gate, typecheck, test, build with telemetry compiled on). This is the required status check for branch protection.

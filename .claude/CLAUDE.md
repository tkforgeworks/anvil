...
- **`ci.yml`** — triggers on pushes to non-master branches and `pull_request` to master. Runs `validate` job (`npm audit --omit=dev --audit-level=high` gate, typecheck, test, build with telemetry compiled on). This is the required status check for branch protection.
- **`release.yml`** — triggers on `push` to master (i.e., after PR merge). Reads version from `package.json`, checks if a `v{version}` tag already exists. If the tag exists → skips silently. If not: shared workflow generates release notes → Windows + Linux matrix builds upload workflow artifacts (telemetry compiled in only on prerelease versions) → a single `publish` job creates a draft GitHub Release with all assets and notes, then flips it to published (immutable-release safe; publishing creates the tag).
...
**Release notes are generated by the shared org standard** — `tkforgeworks/.github` hosts the canonical `scripts/generate-release-notes.js` and a `release-notes.yml` reusable workflow (`workflow_call`). Anvil consumes it with `ticket-prefix: ANV` and an explicit `release-version` (required because anvil's push-to-master flow runs before the tag exists; tag-triggered repos like claude-observability-gui omit it). There is no local copy of the script — changes to note format belong in the standards repo, where they apply to all adopters.

**Commit subjects are the changelog contract.** Notes are built from commit subjects since the previous tag (previous *stable* tag for stable releases, so final notes span all RCs). Version-bump and merge commits are filtered out. Subjects starting with `Fix` or `ANV-N: Fix ...` land under "Bug Fixes", everything else under "Changes"; `ANV-N` keys become Jira links. Write commit subjects as user-readable changelog lines, prefixed with the ticket key when there is one.
...
2. **Cut an RC**: `npm run rc:patch|minor|major` from the integration branch — bumps `package.json` to the next `-rc.N` (no local git tags), commits, pushes, opens a PR to master. (The script only auto-creates a separate `release/v{version}` branch when run from master/main; on an integration branch it commits the bump directly there.)
3. **Merge that PR** → `release.yml` publishes the `v{X.Y.Z}-rc.N` prerelease. Repeat 1–3 for further RCs.
4. **Finalize**: `npm run release:final` promotes the current RC to its stable version (`0.1.4-rc.2` → `0.1.4`; errors if the current version isn't an RC). Merge its PR → stable release.
5. `release:patch|minor|major` are **only** for direct stable releases with no RC phase — they strip any rc suffix and bump *past* it, so never use them to finalize an RC cycle.
...
**npm audit — react-router v6 moderate advisories (accepted)**
The CI audit gate runs `npm audit --omit=dev --audit-level=high`. Two moderate advisories against react-router v6 (open redirect via `<Link>`/`useNavigate`, SSR `deserializeErrors` injection) are accepted: both require a server/web context and do not apply to a desktop Electron app using `HashRouter` with no SSR. The real fix is react-router 8.x, which requires React ≥19.2 — deferred until a React 19 upgrade is scheduled (MUI/recharts compat must be verified then). Do not bump react-router-dom to 7.x: the 7.x line is flagged with *high*-severity advisories and would fail the gate.

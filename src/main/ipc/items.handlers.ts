...
export function registerItemsHandlers(): void {
  safeHandle(IPC_CHANNELS.ITEMS_LIST, (_event, options?: { includeDeleted?: boolean; deletedOnly?: boolean }) =>
    itemRepository.list(options?.includeDeleted ?? false, options?.deletedOnly ?? false),
  )

  safeHandle(IPC_CHANNELS.ITEMS_GET, (_event, id: string) => itemRepository.get(id))

  safeHandle(IPC_CHANNELS.ITEMS_CREATE, (_event, data: CreateItemInput) => {
  ...
  safeHandle(IPC_CHANNELS.ITEMS_UPDATE, (_event, id: string, data: UpdateItemInput) => {
  ...
  safeHandle(IPC_CHANNELS.ITEMS_DELETE, (_event, id: string) => {
  ...
  safeHandle(IPC_CHANNELS.ITEMS_RESTORE, (_event, id: string) => {
  ...
  safeHandle(IPC_CHANNELS.ITEMS_GET_USED_BY, (_event, id: string) =>

...
export function registerAbilitiesHandlers(): void {
  safeHandle(IPC_CHANNELS.ABILITIES_LIST, (_event, options?: { includeDeleted?: boolean; deletedOnly?: boolean }) =>
    abilityRepository.list(options?.includeDeleted ?? false, options?.deletedOnly ?? false),
  )

  safeHandle(IPC_CHANNELS.ABILITIES_GET, (_event, id: string) => abilityRepository.get(id))

  safeHandle(IPC_CHANNELS.ABILITIES_CREATE, (_event, data: CreateAbilityInput) => {
  ...
  safeHandle(IPC_CHANNELS.ABILITIES_UPDATE, (_event, id: string, data: UpdateAbilityInput) => {
  ...
  safeHandle(IPC_CHANNELS.ABILITIES_DELETE, (_event, id: string) => {
  ...
  safeHandle(IPC_CHANNELS.ABILITIES_RESTORE, (_event, id: string) => {
  ...
  safeHandle(IPC_CHANNELS.ABILITIES_GET_USED_BY, (_event, id: string) =>

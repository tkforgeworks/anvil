...
export default function AppSettingsModal(): React.JSX.Element | null {
  ...
  const open = activeModalId === MODAL_IDS.APP_SETTINGS
  ...
  const handleSelectFolder = async (): Promise<void> => {
    ...
    if (folder) void updateSetting({ defaultSaveLocation: folder })
  }
      ...
      <DialogContent sx={{ pt: 3, minHeight: 300 }}>
        {!appSettings ? (
          ...
          <Stack spacing={3} sx={{ maxWidth: 500 }}>
            <Box>
              <Typography variant="subtitle2" gutterBottom>Auto-save</Typography>
              <FormControlLabel
                control={
                  <Switch
                    checked={appSettings.autoSaveEnabled}
                    onChange={(e) => void updateSetting({ autoSaveEnabled: (e.target as HTMLInputElement).checked })}
                    disabled={saving}
                  />
                }
            ...
            <Box>
              ...
              <Stack direction="row" alignItems="center" spacing={1}>
                ...
                <Button size="small" variant="outlined" startIcon={<FolderIcon />} onClick={() => void handleSelectFolder()} disabled={saving}>
                  Browse
                </Button>
                {appSettings.defaultSaveLocation && (
                  <Button size="small" color="secondary" onClick={() => void updateSetting({ defaultSaveLocation: null })} disabled={saving}>
                    Clear
                  </Button>
                )}
              </Stack>
          ...
          <Stack spacing={3} sx={{ maxWidth: 500 }}>
            <Box>
              <Typography variant="subtitle2" gutterBottom>UI Theme</Typography>
              <FormControl>
                <RadioGroup
                  value={appSettings.theme}
                  onChange={(e) => void updateSetting({ theme: e.target.value as AppSettings['theme'] })}
                >
              ...
              {appSettings.theme === 'custom' && (
                <Box sx={{ mt: 1.5, pl: 1, borderLeft: 2, borderColor: 'divider' }}>
                  ...
                  {themeParseError && <Alert severity="error" sx={{ mt: 1 }}>{themeParseError}</Alert>}
                  <CustomThemeGuide />
                </Box>
              )}
            </Box>
            ...
            <Box>
              ...
              <FormControl>
                <RadioGroup
                  ...
                  onChange={(e) => void updateSetting({ editingMode: e.target.value as AppSettings['editingMode'] })}
                >
